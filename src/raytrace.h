...
class PointLight {
public:
  PointLight(const Vec3d &p, const Vec3d &c, const double &i)
      : pos(p), col(c), intensity(i) {}
  Vec3d pos;
  Vec3d col;
  double intensity;
};
...
public:
  Scene() {
    mObjects.push_back(
        std::make_shared<Plane<TextureCheck>>(Vec3d(0, 1, 0), Vec3d(0, -2, 0)));
    // mObjects.push_back(std::make_shared<Cube>(Vec3d(1.5, 1.5, 1.5)));
    // mObjects.push_back(std::make_shared<Torus>(Vec3d(2, 0.5, 0.0), 2, 0.65));
    //
    // mObjects.push_back(std::make_shared<Sphere<TextureCheck>>(Vec3d(-3, 0.0,
    // 0.0), 2));
    // mObjects.push_back(std::make_shared<Sphere<TextureCheck>>(Vec3d(3, 0.0,
    // 0.0), 1));
    ...
    mLights.push_back(std::make_unique<PointLight>(
        Vec3d(20, 10, 20), Vec3d(1.0, 0.2, 0.2), 10000));
    mLights.push_back(std::make_unique<PointLight>(Vec3d(0, 10, 20),
                                                   Vec3d(0.2, 1, 0.2), 5000));
    mLights.push_back(std::make_unique<PointLight>(
        Vec3d(-20, 10, 20), Vec3d(0.2, 0.2, 1.0), 10000));
    mLights.push_back(
        std::make_unique<PointLight>(Vec3d(0, 10, -20), Vec3d(1, 1, 1), 10000));
...
class RayTrace {
public:
  inline float traceShadow(const Vec3d &rayOrigin, const Vec3d &rayDirection,
    ...
    if (minDistance != std::numeric_limits<double>::max()) {
      ...
      for (const auto &light : mScene.mLights) {
        ...
        if (cosAngle > 0) {
          ...
          // shadow
          const float shadow =
              1.0f - traceShadow(isectPoint, lightDir, dist, isectShape);
          R += shadow * cosAngle * light->col * light->intensity /
               (decayScale * dist);
        }
  ...
  void draw(auto &image) {
    ...
    Eigen::Matrix<double, 3, 3> camRotate =
        mScene.mCamToWorld.topRows(3).transpose();
    ...
    for (uint32_t j = 0; j < height; ++j) {
      ...
      for (uint32_t i = 0; i < width; ++i) {
        ...
        image.image[image.POS(i, j)] =
            Pixel{std::min<float>(1.0, pixelColor(0)),
