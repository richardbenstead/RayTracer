...
public:
  ...
  // intersection point.
  virtual void getSurfaceData([[maybe_unused]] const Vec3f &point, Vec3f &norm,
                              Vec3f &col) const = 0;
...
public:
  ...
  bool intersect(const Vec3f &orig, const Vec3f &dir, float &t) const {
    ...
    const float tca = L.dotProduct(dir); // project L along dir to get distance
  ...
  void getSurfaceData([[maybe_unused]] const Vec3f &point, Vec3f &norm,
                      Vec3f &col) const {
...
public:
  Plane(const Vec3f &_normal = Vec3f(0, 1, 0), const Vec3f &pp = Vec3f(0))
...
public:
  Torus(const Vec3f &_center, const float &_r0, const float &_r1)
  ...
  float getDistance(const Vec3f &from) const {
    ...
    const float tmpx = sqrtf(pointObjectSpace.x * pointObjectSpace.x +
  ...
  void getSurfaceData([[maybe_unused]] const Vec3f &point, Vec3f &norm,
    ...
    norm = Vec3f(getDistance(point + Vec3f(delta, 0, 0)) - base,
                 ...
                 getDistance(point + Vec3f(0, 0, delta)) - base)
               .normalize();
    col = color;
  }
  const Vec3f center;
...
public:
  ...
  inline float getDistance(const Vec3f &point) const {
    ...
    float dmaxLen = sqrt(d.x * d.x * (d.x > 0) + d.y * d.y * (d.y > 0) +
                         d.z * d.z * (d.z > 0));
...
struct mixFunc {
  ...
  float operator()(float a, float b) const { return a * (1 - t) + b * t; }
  float t;
};
...
public:
  SoftObject() {
    ...
    for (size_t i = 0; i < 20; ++i) {
      ...
      Vec3f c((0.5 - drand48()) * 3, (0.5 - drand48()) * 3,
              (0.5 - drand48()) * 3);
...
template <typename S1, typename S2> using Blend = CSG<blendFunc, S1, S2, float>;

template <typename S1, typename S2> using Mix = CSG<mixFunc, S1, S2, float>;
