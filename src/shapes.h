...
template <typename TEX_T = TextureCheck> class Sphere : public ImplicitShape {
  public:
    Sphere(const Vec3d &c, const double &r, const Vec3d &col = Vec3d(1, 1, 1))
        : ImplicitShape(col), center{c}, radius{r}, radius2{r * r} {}

    double getDistance(const Vec3d &from) const { return (from - center).norm() - radius; }
    ...
    void getSurfaceData([[maybe_unused]] const Vec3d &point, Vec3d &normal, Vec3d &col) const {
        ...
        col = color * texture.getVal((1 + atan2(normal[2], normal[0]) / M_PI) * 0.5, acosf(normal[1]) / M_PI);
    }
    const TEX_T texture{};
    const Vec3d center;
    const double radius;
...
template <typename TEX_T = TextureCheck> class Plane : public ImplicitShape {
  public:
    Plane(const Vec3d &_normal = Vec3d(0, 1, 0), const Vec3d &pp = Vec3d(0), const Vec3d &col = Vec3d(1, 1, 1))
    ...
    void getSurfaceData([[maybe_unused]] const Vec3d &point, Vec3d &norm, Vec3d &col) const {
        norm = normal;
...
class MarchingCubeImplicitShape : public ImplicitShape {
  public:
    ...
    // details
    virtual void getSurfaceData([[maybe_unused]] const Vec3d &point, Vec3d &normal, Vec3d &col) const {
        constexpr double delta = 10e-5;
        const double base = getDistance(point);
        normal = Vec3d(getDistance(point + Vec3d(delta, 0, 0)) - base, getDistance(point + Vec3d(0, delta, 0)) - base,
...
class Torus : public MarchingCubeImplicitShape {
  public:
    Torus(const Vec3d &_center, const double &_r0, const double &_r1, const Vec3d &col = Vec3d(1, 1, 1))
    ...
    virtual double getDistance(const Vec3d &from) const {
        ...
        const double tmpx =
            fastSqrt(pointObjectSpace[0] * pointObjectSpace[0] + pointObjectSpace[2] * pointObjectSpace[2]) - r0;
...
class Cube : public MarchingCubeImplicitShape {
  public:
    Cube(const Vec3d &_corner, Vec3d col = Vec3d(1, 1, 1)) : MarchingCubeImplicitShape(col), corner(_corner) {}
...
template <typename Op, typename S1, typename S2, typename... Args> class CSG : public MarchingCubeImplicitShape {
  public:
    CSG(const S1 &s1, const S2 &s2, Args &&... args) : op(std::forward<Args>(args)...), shape1(s1), shape2(s2) {}
    double getDistance(const Vec3d &from) const { return op(shape1.getDistance(from), shape2.getDistance(from)); }
    Op op;
    const S1 shape1;
    const S2 shape2;
};
...
std::pair<double, double> smin(double a, double b, double k) {
    ...
    return (a < b) ? std::pair<double, double>(a - s, m) : std::pair<double, double>(b - s, 1.0 - m);
}
  ...
  public:
    ...
    SoftObject() : ImplicitShape(Vec3d(1, 1, 1)) {
        ...
        for (size_t i = 0; i < 5; ++i) {
            ...
            Vec3d c((0.5 - drand48()) * 3, (0.5 - drand48()) * 3, (0.5 - drand48()) * 3);
...
template <typename S1, typename S2> using Intersect = CSG<intersectionFunc, S1, S2>;
